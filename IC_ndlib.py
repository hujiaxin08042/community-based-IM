...
class ICModel:
    ...
    def inf_pp_random_threshold(self):
        ...
        config.add_model_initial_configuration('Infected', self.seeds)
        
        for u, v, d in self.graph.edges(data='weight'):
            config.add_edge_configuration("threshold", (u, v), d)
        
        model.set_initial_status(config)
        
        count = 0
        status_len = self.graph.number_of_nodes()

        while status_len:
            iteration = model.iteration()
            count += iteration['node_count'][1]
            status_len = len(iteration['status'])
    ...
    def inf_pp_fixed_random_threshold(self):
        ...
        for e in self.graph.edges():
            config.add_edge_configuration("threshold", e, self.p)
