...
import os
from IC import IC
from selectInComm_6 import mpCommAI
...
def train_sdcn():
    ...
    for i in range(args.n_clusters):
        ...
        if len(comm) > 0:
            ...
            # 计算属性平均值和Query的余弦相似度
            cosine = feature.dot(query) / (numpy.linalg.norm(feature) * numpy.linalg.norm(query))
    ...
    for line in graph.readlines():
        ...
        vec1 = features[int(n1)]
        vec2 = features[int(n2)]
        cos_sim = vec1.dot(vec2) / (numpy.linalg.norm(vec1) * numpy.linalg.norm(vec2))
        # 如果两个节点属于同一个社区，考虑社区与Query的相似度
        if comm_data[int(n1)] == comm_data[int(n2)]:
            query_sim = distribute[comm_data[int(n1)]]
            pp = cos_sim + query_sim.item()
            similarity.write(n1 + ' ' + n2 + ' ' + str(pp) + '\n')
        else:
...
def im(config, comm_data):
    ...
    # 创建整个网络的图
    G = utils.load_graph_pp(args.name, args.nodeNum)
...
if __name__ == "__main__":
    ...
    f = open('sdcn6Result/sdcn_6_' + args.name + '_' + str(args.k) + '.txt', 'w', encoding='utf-8')
    print('k: ' + str(args.k) + '\n')
