...
class LTModel:
    ...
    def __init__(self, threshold_method='pp_random', graph=None, seeds=None, worker_num=8, simulate_times=10000) -> None:
                ...
                for node in self.graph.neighbors(seed):
                    if node not in active_nodes:
                        if node not in node_threshold:
                            ...
                            node_weights[node] = 0
                        pp = self.graph[seed][node]['weight'] + self.graph.nodes[node]['node_query_sim']
                        node_weights[node] += pp
    ...
    def inf_avg_threshold(self):
        ...
        while activity_set:
            new_activity_set = set()
            for seed in activity_set:
                for node, weight in self.graph.get_neighbors(seed):
                    if node not in active_nodes:
                        if node not in node_threshold:
...
def LT(g, S, p=0.1, mc=10000, method='random'):
    # graph: network 图
    # S: 种子集
    # p: 影响概率
    # mc: 模拟次数
    ...
    model = LTModel(threshold_method=method, graph=g, seeds=S, worker_num=8, simulate_times=mc)
    influence = model.calculate_influence()
    return influence

if __name__ == "__main__":
    # 随机生成维数为属性维数的query
    query = numpy.random.dirichlet(numpy.ones(334), size=1).reshape(334,)
    # 计算每一个节点与Query的相似度
    features = numpy.loadtxt(os.path.dirname(os.path.abspath(__file__)) + '/data/' + 'dblp' + '.txt', dtype=float)
    node_query_sim = [feature.dot(query) / (numpy.linalg.norm(feature) * numpy.linalg.norm(query)) for feature in features]
    G = utils.load_graph_query('dblp', 4057, node_query_sim)
