...
import os
from IC import IC
from selectInComm import mpCommAI
...
def train_sdcn(config):
    ...
    seeds_list = mpCommAI(comm_data, config["n_clusters"], args.name, args.nodeNum, args.k, query, config["ratio"])
    spreadSum = IC(graph, seeds_list, mc=10000, method='pp_random')
    session.report({"loss": loss.item(), "spreadSum": spreadSum, "seeds_list": seeds_list})
...
def main():
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(train_sdcn),
...
if __name__ == "__main__":
    ...
    start = time.time()
    seeds, spreadSum = main()
    end = time.time()

    f = open('SDCNResult/SDCN_' + args.name + '_' + str(args.k) + '.txt', 'w', encoding='utf-8')
    print('k: ' + str(args.k) + '\n')
