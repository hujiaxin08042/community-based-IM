...
import os
from IC import IC
from selectInComm_3 import mpCommAI
...
def main(res3):
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            tune.with_parameters(im, res3=res3),
...
if __name__ == "__main__":
    ...
    start = time.time()
    res3 = train_sdcn()
    seeds, spreadSum = main(res3)
    end = time.time()

    print('dataset: ' + str(args.name))
    f = open('sdcn3Result/sdcn_3_' + args.name + '_' + str(args.k) + '.txt', 'w', encoding='utf-8')
    print('k: ' + str(args.k) + '\n')
