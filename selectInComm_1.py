...
# 获取社区，仅根据社区大小来分配点数
def load_comm(data, n_clusters, dataset, nodeNum, k):
    communities = []
    select_num = []
    commSize = []
...
# 在每个社区内使用AI选点，进程并行
def mpCommAI(data, n_clusters, dataset, nodeNum, k):
    communities, select_num = load_comm(data, n_clusters, dataset, nodeNum, k)
    worker = []
